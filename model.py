...
from utils_graph import Node, get_graph_info, build_graph, save_graph, load_graph
import torch
import math
import os
...
class CNN(nn.Module):
  def __init__(self, args, num_classes=1000):
    ...
    self.conv1 = depthwise_separable_conv_3x3(3, args.channels // 2, 2)    # nin, nout, stride
    self.bn1 = nn.BatchNorm2d(args.channels // 2)
    
    # 채널수 변화도, 논문에서처럼 conv2: C, conv3: 2C, conv4: 4C, conv5: 8C    
    if args.net_type == 'small':
      self.conv2 = Triplet_unit(args.channels // 2, args.channels, 2)    # inplanes, outplanes, stride=2
      ...
      self.relu = nn.ReLU()
      self.conv = nn.Conv2d(args.channels * 4, 1280, kernel_size=1)   # 마지막에 1x1 conv, 1280-d
